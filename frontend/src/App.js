...
import ModalGrade from "./components/ModalGrade/ModalGrade";
...
export default function App() {
  ...
  const handleDelete = async (gradeToDeleted) => {
    const isDeleted = await api.deleteGrade(gradeToDeleted);

    if (isDeleted) {
      const deletedGradeIndex = allGrades.findIndex(
        (grade) => grade.id === gradeToDeleted.id
      );
